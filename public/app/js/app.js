...
var App = angular.module('angle', ['ngRoute', 'ngAnimate', 'ngStorage', 'ngCookies', 'pascalprecht.translate', 'ui.bootstrap', 'ui.router', 'oc.lazyLoad', 'cfp.loadingBar', 'ngSanitize', 'ngResource', 'ui.utils'])
          .run(["$rootScope", "$state", "$stateParams",  '$window', '$templateCache','$http','$location', function ($rootScope, $state, $stateParams, $window, $templateCache,$http,$location) {
             ...
             $rootScope.$storage = $window.localStorage;
              ...
              // ----------------------------------- 
              $rootScope.app = {
                name: 'DeviceSimualtor',
                description: 'Device Simualtor example',
...
App.filter('FormatNumber', function() {
	  return function(input) {
	    //console.log("Filter Called" + String(input).length)
	    if(String(input).length <= 1){
	    	//console.log("input less");
	 ...
	 $scope.restartsimulation =function(){
		 ...
		 swal({
			...
			function(){
				 $http({ 
					  method: 'GET',
					  url: 'http://'+$location.host()+':'+$location.port()+'/StartSimulation'
					}).then(function successCallback(response) {
					    // this callback will be called asynchronously
					    // when the response is available
						//swal("Washing machine simulator successfully restarted!");
						swal("Factory reset!", "Washing machine simulator successfully reset.", "success");
...
App.controller('DashboardController', ['$scope', '$http', '$state','$timeout','$rootScope','$location', function($scope, $http, $state,$timeout,$rootScope,$location) {
	...
	var Wisconsinwatergauge=null;
	 var Washingtontempgauge=null;
	 var Washingtonwatergauge=null;
	 ...
	 $scope.selectWashingmachine =function(index){
		 $scope.data=$scope.Devices[index];
 ...
 if ("WebSocket" in window)
     {
       // alert("WebSocket is supported by your Browser!");
        ...
        var ws = new WebSocket("ws://"+$location.host()+":"+$location.port()+"/ws/data");
...
App.controller('SidebarController', ['$rootScope', '$scope', '$state', '$http', '$timeout', 'Utils',
  function($rootScope, $scope, $state, $http, $timeout, Utils){
    ...
    $scope.idSelected = $state.current.title;
    ...
    $rootScope.$on('$stateChangeStart', 
    	...
    	$scope.idSelected = toState.title;
   ...
   /* console.log($state.current)*/
    ...
    /*
    $scope.DropdownName=$state.current
    {"text":"Dashboard","sref":"app.dashboard","icon":"icon-speedometer","translate":"sidebar.nav.DASHBOARD"}
*/
    

    $scope.itemclicked = function(item) {
       console.log(item);
       $scope.DropdownName=item;
      };
      ...
      $http.get(menuURL)
        .success(function(items) {
           ...
           for(var i=0;i <items.length;i++){
        	   ...
        	   if(items[i].text == $state.current.title){
              		
        		   /*console.log(items[i]); */
              		 $scope.DropdownName=items[i];
              	}
           }
        })
